...
> 위 GitHub Pages 주소는 저장소 이름을 기준으로 작성한 기본 프로젝트 사이트 주소 형식입니다. 실제 배포 완료 여부와 접속은 확인하지 않았으므로, GitHub의 배포 화면에 표시된 URL 및 정상 접속 여부를 확인한 뒤 제출합니다.
...
아래 항목은 구현 여부와 별개로 실제 테스트 결과를 기록하기 위한 목록입니다. 확인 후 `[ ]`를 `[x]`로 변경합니다.

- [ ] 기존 반응형·메뉴·스크롤·다크 모드·폼 기능 회귀 테스트
- [ ] API 응답에 맞는 언어 필터 버튼 생성
- [ ] 특정 언어 선택 시 일치하는 저장소만 표시
- [ ] “전체” 선택 시 불러온 전체 저장소 복원
- [ ] 언어가 `null`인 저장소가 “전체”에 표시됨
- [ ] 필터 선택 상태와 `aria-pressed` 일치
- [ ] 필터 클릭 시 불필요한 추가 API 요청이 발생하지 않음
- [ ] 오류 상태에서 “다시 시도”로 정상 상태 복구
- [ ] HTTP 403 요청 한도 오류와 일반 접근 거부 분기
- [ ] HTTP 500 이상 및 네트워크 오류 분기
- [ ] 키보드 Tab 이동과 포커스 표시
- [ ] Escape 키로 모바일 메뉴 닫기
- [ ] 키보드로 필터를 연속 선택할 때 포커스 유지 여부
- [ ] 움직임 감소 설정에서 스크롤·애니메이션 동작
- [ ] 저장소 카드가 많을 때 Projects 섹션 등장 여부
- [ ] 실제 스크린리더에서 메뉴·필터·폼 상태 안내 확인
- [ ] GitHub Pages 배포 후 이미지·CSS·JS 상대 경로 확인
- [ ] 배포 사이트에서 전체 기능 재확인

API 오류 테스트를 위해 요청을 의도적으로 대량 전송하지 않습니다. 테스트용 변경을 사용했다면 테스트 후 원래 사용자명과 `await response.json()` 코드로 복구합니다.
